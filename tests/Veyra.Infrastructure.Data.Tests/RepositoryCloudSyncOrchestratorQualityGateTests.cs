...
public sealed class RepositoryCloudSyncOrchestratorQualityGateTests
{
    ...
    [Fact]
    public async Task ProcessPendingQueue_SignsOutActiveProfileWhenRefreshSessionIsRejected()
    {
        await using var scope = await SqliteDbScope.CreateAsync();
        var blockStoreRoot = CreateTempDirectory();
        ...
        try
        {
            var userProfiles = new FakeUserProfileRepository();
            var auth = new FakeAuthService(
                refreshException: new CloudAuthRefreshRejectedException(
                    HttpStatusCode.Unauthorized,
                    "invalid session",
                    "{\"ok\":false,\"message\":\"invalid session\"}"));
            var tokenPolicy = new FakeAccessTokenPolicyService(canUseForSync: false);
            var orchestrator = CreateOrchestrator(
                scope.Db,
                blockStoreRoot,
                new AlwaysFailPushCloudSyncService(),
                auth,
                userProfiles,
                tokenPolicy);
            ...
            Assert.True(userProfiles.SignOutCalled);
            Assert.Null(await userProfiles.GetActiveProfileAsync());
        }
        finally
        {
            DeleteDirectoryQuietly(blockStoreRoot);
    ...
    private static RepositoryCloudSyncOrchestrator CreateOrchestrator(
        ...
        IAccessTokenPolicyService? tokenPolicy = null)
    {
        ...
        return new RepositoryCloudSyncOrchestrator(
            ...
            tokenPolicy ?? new FakeAccessTokenPolicyService(),
    ...
    {
        private UserProfileSessionDto? _profile = new(
            Username: "tester",
        ...
        public Task<string?> GetActiveUsernameAsync(CancellationToken ct = default)
            => Task.FromResult(_profile?.Username);
        ...
        public Task<IReadOnlyList<UserProfileSessionDto>> GetProfilesAsync(CancellationToken ct = default)
            => Task.FromResult<IReadOnlyList<UserProfileSessionDto>>(_profile is null ? [] : [_profile]);
    ...
    private sealed class FakeAccessTokenPolicyService(bool canUseForSync = true) : IAccessTokenPolicyService
    {
        public AccessTokenPolicyEvaluationDto Evaluate(string? accessToken, DateTime? nowUtc = null)
            => new(
                canUseForSync ? AccessTokenValidityState.Valid : AccessTokenValidityState.Expired,
                ExpiresAtUtc: canUseForSync ? DateTime.UtcNow.AddHours(1) : DateTime.UtcNow.AddMinutes(-5),
                RemainingLifetime: canUseForSync ? TimeSpan.FromHours(1) : TimeSpan.FromMinutes(-5),
                CanUseForSync: canUseForSync,
                Description: canUseForSync ? "valid" : "expired");

        public string GetPolicySummary() => "test";
    }

    private sealed class FakeAuthService(Exception? refreshException = null) : IAuthService
    {
    ...
    private sealed class FakeFileContentStore : IFileContentStore
    {
        ...
        public Task<long> RestoreFileAsync(IReadOnlyList<StoredFileBlockDto> blocks, string targetPath, bool overwriteExisting, string? expectedContentHash = null, CancellationToken ct = default)
            => throw new NotSupportedException();
    ...
    {
        public CloudAvailabilitySnapshot Snapshot { get; private set; } = new(
            CloudAvailabilityState.Online,
            DateTime.UtcNow,
            RetryAfterUtc: null);
        ...
        {
            Snapshot = new CloudAvailabilitySnapshot(
                CloudAvailabilityState.Online,
                DateTime.UtcNow,
                RetryAfterUtc: null);
        }
    ...
    private sealed class CheckpointCloudSyncService(string blockHashA, string blockHashB) : ICloudSyncService
    {
        ...
        public Task<IReadOnlyList<CloudSnapshotPackageDto>> GetRepositorySnapshotsAsync(string accessToken, int repositoryId, CancellationToken ct = default)
            => Task.FromResult<IReadOnlyList<CloudSnapshotPackageDto>>([]);
