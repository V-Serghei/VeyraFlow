...
pub extern "C" fn veyra_zstd_compress(
    ...
    let compressed = match compress::zstd_compress_buffer(input, level) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(&e);
            return -1;
...
pub extern "C" fn veyra_zstd_decompress(
    ...
    let decompressed = match compress::zstd_decompress_buffer(input) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(&e);
            return -1;
...
pub extern "C" fn veyra_zstd_compress_file(
    ...
    match compress::zstd_compress_file(src_path, dst_path, level) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(&e);
            -1
...
pub extern "C" fn veyra_zstd_decompress_file(
    ...
    match compress::zstd_decompress_file(src_path, dst_path) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(&e);
            -1
