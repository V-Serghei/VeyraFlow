# VeyraFlow — Codex Knowledge Base

> This file is loaded automatically on every Codex session in this project.
> Detailed documentation is in `.Codex/docs/`. Slash commands are in `.Codex/commands/`.
...
### Agent docs (`.Codex/docs/`) — quick reference
- [Architecture](.Codex/docs/architecture.md) — layers, dependencies, DI
- [Domain entities](.Codex/docs/domain.md) — all entities and their relations
- [Offline & guest mode](.Codex/docs/offline.md) — offline-first behavior, guest mode rules, cloud availability, CanSyncToCloud, connectivity service
- [Code patterns](.Codex/docs/patterns.md) — command, query, VM templates
- [Known issues](.Codex/docs/gotchas.md) — common errors and fixes
- [Avalonia tips](.Codex/docs/avalonia.md) — UI framework specifics
...
1. Update the relevant file in `.Codex/docs/`
2. Recurring error → add to `.Codex/docs/gotchas.md`
3. New pattern → add to `.Codex/docs/patterns.md`
4. New command → create `.Codex/commands/<name>.md`
5. Update `MEMORY.md` (agent memory index) in `.Codex/` or in `C:\Users\visto\.Codex\projects\...`

### Rule: "skills always up to date"
- If you implemented a new feature — document how it works in `.Codex/docs/`
