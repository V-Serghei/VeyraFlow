...
internal static class DatabaseStartupBootstrapper
{
    ...
    public static void Initialize(VeyraDbContext db)
    {
        ...
        db.Database.ExecuteSqlRaw($"PRAGMA busy_timeout={SqliteRuntimeDefaults.BusyTimeoutMilliseconds};");
...
CREATE TABLE IF NOT EXISTS ""OperationJournalEntries"" (
    ...
    private static void EnsureRepositoryRetentionColumns(VeyraDbContext db)
    {
        ...
        try
        {
            ...
            EnsureSqliteColumnExists(connection, "Repositories", "RetentionPolicyOverrideEnabled", "INTEGER NOT NULL DEFAULT 0");
            ...
            using var backfillOverride = connection.CreateCommand();
            backfillOverride.CommandText = """
                UPDATE "Repositories"
                ...
                """;
            backfillOverride.ExecuteNonQuery();
